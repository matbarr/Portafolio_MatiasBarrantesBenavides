...
public class CarritoService {
    ...
    public Factura procesarCompra(List<Item> carrito, Usuario usuario) {
        ...
        if (usuario == null) {
            throw new RuntimeException("No hay un usuario válido para procesar la compra.");
        }
        ...
        for (Item item : carrito) {
            ...
            Producto producto = productoRepository.findById(item.getProducto().getIdProducto())
                    .orElseThrow(() -> new RuntimeException("Producto no encontrado durante el procesamiento de compra."));
